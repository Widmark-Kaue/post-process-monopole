...
%% RESULTADOS MONOPOLO ESTACION�RIO
%p_=-1*imag(P*exp(-1i*omega*t));         % Aqui utilizei -Im pois a excita��o � um seno na disserta��o

% Thesis=load('akhnoukh.txt');           % anal�tico da disserta��o extra�do do gr�fico
Simulation = load('datau01_ppw32_t017.txt');        % resultado da simula��o

% figure(1)
% %plot(r,p_(1:length(r)))                                % Plot do anal�tico com fonte da disserta��o
% % plot(Thesis(:,1), Thesis(:,2), 'r-')                  % Anal�tico extra�do do gr�fico
% hold on
% %plot(r,p_2(1:length(r)), 'k-')                          % Anal�tico Akhnoukh com fonte pontual
% %plot(r,p_2_B(1:length(r)), 'k-')                        % Anal�tico Buckingham com fonte pontual
% plot(r,p_2_J(1:length(r)), 'k-')                        % Anal�tico Jacobsen com fonte pontual
% plot(Simulation(:,8), (Simulation(:,5))-101325, 'r-')   % Simula��o
% %plot(Simulation(:,13), (Simulation(:,8))-101325, 'r-') % Simula��o com
% %meta data
% hold off
% xlabel('r (m)')
% ylabel('p (Pa)')
