...
# defining variables
x , y, omega, t, c0, M, alpha, epsilon = \
    sy.symbols('x y omega t c0 M alpha epsilon', real=True) 

G = sy.I/(4*c0**2*sy.sqrt(1-M**2))*\
    sy.hankel1(0,omega*sy.sqrt(x**2+(1-M**2)*y**2)/(c0*(1-M**2)))*\
...
for i in range(len(xrange)):
    for j in range(len(yrange)):
        H_num[i,j]=H.xreplace({x: xrange[i],y: yrange[j],omega: omega_num,t: t_num,\
                               c0: c0_num,M: M_num}).evalf()
        f_num[i,j]=f.xreplace({x: xrange[i],y: yrange[j], alpha: alpha_num,\
                         epsilon: epsilon_num}).evalf()
...
data=createDict('X','Y','pprime','gamma','T0','p0','R','omega_num','c0_num',\
                'alpha_num','M_num','t_num','epsilon_num','y_num','xlim',\
                    'ylim','nx','ny')
